...
def slow(lines):
    ...
    while uniqueness_dict.keys():
        ...
        uniqueness_dict.pop(least_unique_id)
        ans.append(least_unique_id)
        for k in uniqueness_dict.keys():
            r, g, b = card_dict[k]
            r_uniqueness, r_neighbors = find_uniqueness(red_dict, r, id_ignore=k) ## potential optimization: only update affected color of uniqueness
            g_uniqueness, g_neighbors = find_uniqueness(green_dict, g, id_ignore=k)
            b_uniqueness, b_neighbors = find_uniqueness(blue_dict, b, id_ignore=k)
            uniqueness = r_uniqueness + g_uniqueness + b_uniqueness
            neighbors = r_neighbors + g_neighbors + b_neighbors
            uniqueness_dict[k] = (uniqueness,-k, set(neighbors))
    return ans
...
for _ in range(100):
    lines = genInput(n = 10)
    #lines = ['42 1 1 1', '90 1 1 2','110 1 1 3']
    fastAns = fast(lines)
    
    slowAns = slow(lines)
    
    if not all([fastAns[i]==slowAns[i] for i in range(len(fastAns))]):
        print("RONG")
        print(slowAns)
        print("fast")
        print(fastAns)
